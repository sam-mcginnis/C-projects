...
if [ ! -f Elevate.c ]; then
	echo ""
	echo " Error: You must place Elevate.c in this directory before we can"
	echo "        proceed. Aborting test script."
	echo ""
	exit
elif [ ! -f Elevate.h ]; then
	echo ""
	echo " Error: You must place Elevate.h in this directory before we can"
	echo "        proceed. Aborting test script."
	echo ""
	exit
...
# Save a backup version of Elevate.h to be restored after running this
# script, so that the #define main line ends up back how you had it.
cp Elevate.h Elevate_backup.h
...
run_test_case()
{
	...
	echo -n "  [Test Case] ./a.out $(cat $argument_file) ... "

	# Ensure that Elevate.h is in normal test case mode (not unit test mode).
	perl -p -i -e 's/^#define main/\/\/#define main/' Elevate.h

	# Attempt to compile.
	gcc Elevate.c 2> /dev/null
	compile_val=$?

	# Immediately restore header file in case the user terminates the script
	# before we can move on to the next test case.
	cp Elevate_backup.h Elevate.h
...
run_unit_test()
{
	...
	# Ensure that Elevate.h is in unit test case mode.
	perl -p -i -e 's/^\/\/#define main/#define main/' Elevate.h

	# Attempt to compile.
	gcc Elevate.c UnitTestLauncher.c $testcase_file 2> /dev/null
	compile_val=$?
...
if [ -f Elevate_backup.h ]; then
	mv Elevate_backup.h Elevate.h
fi
